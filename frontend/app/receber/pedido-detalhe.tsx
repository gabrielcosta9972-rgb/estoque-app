...
export default function PedidoDetalhe() {
  ...
  const totalItems = order?.items.reduce((acc, it) => acc + it.quantity, 0) || 0;
      ...
      <Modal visible={editOpen} transparent animationType="slide" onRequestClose={() => setEditOpen(false)}>
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            ...
            <TouchableOpacity
              style={[styles.modalConfirm, acting && { opacity: 0.6 }]}
              onPress={() => receiveOrder(editedItems, adjustmentNote)}
              disabled={acting}
              activeOpacity={0.85}
            >
              <Text style={styles.modalConfirmText}>Salvar e confirmar</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
